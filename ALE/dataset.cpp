...
LSowerbyDataset::LSowerbyDataset() : LDataset()
{
	...
	textonClusteringTrainFile = "textonclustering.txt";
	textonFolder = "Sowerby/Result/Feature/Texton/";
	textonExtension = ".txn";

	siftClusteringTrainFile = "siftclustering.txt";
	...
	colourSiftClusteringTrainFile = "coloursiftclustering.txt";
	...
	lbpClusteringFile = "lbpclustering.txt";
	...
	denseBoostTrainFile = "denseboost.txt";
	denseExtension = ".dns";
...
LCorelDataset::LCorelDataset() : LDataset()
{
	...
	cooccurenceTrainFile = "cooccurence.txt";
...
//	random forest for unary pixel potential (replace previous 3 lines)
	const char *denseForestFile = "denseforest.txt";
	int denseTrees = 20;
	int denseDepth = 10;
	double denseForestDataRatio = 1.0;

	LRandomForest<int> *pixelForest = new LRandomForest<int>(trainFolder, "denseforest.txt", classNo, denseTrees, denseDepth, denseThetaStart, denseThetaIncrement, denseNumberOfThetas, denseRandomizationFactor, pixelPotential, (int *(LPotential::*)(int, int *, int, int))&LDenseUnaryPixelPotential::GetTrainForestValues, (int (LPotential::*)(int, int))&LDenseUnaryPixelPotential::GetEvalForestValue, denseForestDataRatio);
...
//	linear svm for unary segment potential (replace previous boosting lines)
	const char *statsLinSVMFile = "statslinsvm.txt";
...
//	approx kernel svm for unary segment potential (replace previous boosting lines)
	const char *statsApproxSVMFile = "statsapproxsvm.txt";
...
//	random forest for unary segment potential (replace previous boosting lines)
	const char *statsForestFile = "statsforest.txt";
	int statsTrees = 50;
	int statsDepth = 12;
...
LMsrcDataset::LMsrcDataset() : LDataset()
{
	...
	denseRandomizationFactor = 0.003;
	denseBoostTrainFile = "denseboostXX.txt";
	denseExtension = ".dns";
...
LLeuvenDataset::LLeuvenDataset() : LDataset()
{
	...
	imageExtension = ".png";
	groundTruthFolder = "Leuven/Object/";
	groundTruthExtension = ".png";
	...
	cameraBaseline = 540;
	cameraHeight = 370;
	cameraFocalLength = 781;
	cameraAspectRatio = 0.3613;
...
LKittiDataset::LKittiDataset() : LDataset()
{
	...
	imageFolder = "Kitti/Images/Left/";
	flowFolder = "Kitti/Flow/Left/";
	deepflow = "LDOF/DeepFlow/deepflow";
	datasetName = "kitti";
	imageExtension = ".png";
	groundTruthFolder = "Kitti/Object/";
	groundTruthExtension = ".png";
	...
	denseBoostTrainFile = "denseboost.txt";
	denseExtension = ".dns";
	denseFolder = "Kitti/Result/Dense/";
	denseWeight = 1.0;
	denseMaxClassRatio = 0.1;
    ...
    motionTestFolder = "Kitti/Result/";
    motionClassNo=2;
    motionUnaryFactor=100;
    motionFolder = "Kitti/Result/motion/";
    motionExtension = ".csv";
    
    dispTestFolder = "Kitti/Result/DispCrf/";
	disparityLeftFolder = "Kitti/Images/Left/";
	disparityRightFolder = "Kitti/Images/Right/";
	disparityGroundTruthFolder = "Kitti/Depth/";
	disparityGroundTruthExtension = ".png";
	disparityUnaryFactor = 0.1;
	disparityRangeMoveSize = 0;

	disparityFilterSigma = 5.0;
	disparityMaxDistance = 1000;
	disparityDistanceBeta = 500;
	disparitySubSample = 1;
	disparityMaxDelta = 2;

	disparityPairwiseFactor = 0.000005;//0.00005;
	disparityPairwiseTruncation = 10.0;
	disparityClassNo = 100;

	cameraBaseline = 0.5707;
	cameraHeight = 1.65;
	cameraU=607.19;
	cameraV=185.21;
	cameraFocalLength = 718.85;
	cameraAspectRatio = 0.3613;
	cameraWidthOffset = -2.0;
	cameraHeightOffset = -3.0;

	crossUnaryWeight = 0.5;
	crossPairwiseWeight = 1;//-1e-4;
	crossMinHeight = 0;
	crossMaxHeight = 1000;
	crossHeightClusters = 80;
	crossThreshold = 1e-6;
...
void LKittiDataset::DisparityRgbToLabel(unsigned char *rgb, unsigned char *label)
{
	if(rgb[0] == 255) label[0] = 0;
	else label[0] = rgb[0] + 1;
}

void LKittiDataset::DisparityLabelToRgb(unsigned char *label, unsigned char *rgb)
{
	rgb[0] = rgb[1] = rgb[2] = label[0] - 1;

}

void LKittiDataset::motionRgbToLabel(unsigned char *rgb, unsigned char *label)
{
	if(rgb[0] == 255) label[0] = 2;
	else label[0] = 1;
}

void LKittiDataset::motionLabelToRgb(unsigned char *label, unsigned char *rgb)
{
	double check=label[0];
if (check==2)
...
// 	{
// 		segmentation[i] = new LMeanShiftSegmentation2D(meanShiftXY[i], meanShiftLuv[i], meanShiftMinRegion[i], meanShiftFolder[i], meanShiftExtension);
// 		superpixelLayer[i] = new LPnCrfLayer(crf, motionDomain, this, motionBaseLayer, segmentation[i], cliqueTruncation);
// 		crf->segmentations.Add(segmentation[i]);
// 		crf->layers.Add(superpixelLayer[i]);
// 	}

    //LHeightUnaryPixelPotential *heightPotential = new LHeightUnaryPixelPotential(this, crf, objDomain, baseLayer, motionDomain, motionBaseLayer, trainFolder, crossTrainFile, classNo, crossUnaryWeight, disparityClassNo, cameraBaseline, cameraHeight, cameraFocalLength, cameraAspectRatio, cameraWidthOffset, cameraHeightOffset, crossMinHeight, crossMaxHeight, crossHeightClusters, crossThreshold, disparitySubSample);
	  //crf->potentials.Add(heightPotential);
	LDenseUnaryPixelPotential *pixelPotential = new LDenseUnaryPixelPotential(this, crf, objDomain, baseLayer, trainFolder, denseBoostTrainFile, denseFolder, denseExtension, classNo, denseWeight, denseBoostingSubSample, denseNumberOfRectangles, denseMinimumRectangleSize, denseMaximumRectangleSize, denseMaxClassRatio);
    pixelPotential->AddFeature(textonFeature);
    pixelPotential->AddFeature(siftFeature);
	...
	crf->potentials.Add(statsPotential);
 	...
 	LCrfDomain *dispDomain = new LCrfDomain(crf, this, disparityClassNo, dispTestFolder, (void (LDataset::*)(unsigned char *,unsigned char *))&LKittiDataset::DisparityRgbToLabel, (void (LDataset::*)(unsigned char *,unsigned char *))&LKittiDataset::DisparityLabelToRgb);
 	crf->domains.Add(dispDomain);
...
//      LCrfDomain *motionDomain = new LCrfDomain(crf, this, motionClassNo, motionTestFolder, (void (LDataset::*)(unsigned char *,unsigned char *))&LKittiDataset::motionRgbToLabel, (void (LDataset::*)(unsigned char *,unsigned char *))&LKittiDataset::motionLabelToRgb);
// 	crf->domains.Add(motionDomain);
// 
// 	LBaseCrfLayer *motionBaseLayer = new LBaseCrfLayer(crf, motionDomain, this, 0);
// 	crf->layers.Add(motionBaseLayer);

//    LmotionJointUnaryPixelPotential *jointmotionPotential = new LmotionJointUnaryPixelPotential(this, crf, objDomain, baseLayer, motionDomain, motionBaseLayer, trainFolder, crossTrainFile, classNo, crossUnaryWeight, motionClassNo, cameraBaseline, cameraHeight, cameraFocalLength, cameraAspectRatio, cameraWidthOffset, cameraHeightOffset, crossMinHeight, crossMaxHeight, crossHeightClusters, crossThreshold, disparitySubSample,motionFolder);
// 	crf->potentials.Add(jointmotionPotential);
// 	LMotionUnaryPixelPotential *pixelmotionPotential = new LMotionUnaryPixelPotential(this, crf, motionDomain, motionBaseLayer, motionClassNo, motionUnaryFactor,motionFolder);
 //	crf->potentials.Add(pixelmotionPotential);    
////    crf->potentials.Add(new LEightNeighbourPottsPairwisePixelPotential(this, crf, motionDomain, motionBaseLayer, motionClassNo,pairwisePrior, pairwiseFactor, pairwiseBeta, pairwiseLWeight, pairwiseUWeight, pairwiseVWeight));
  //  crf->potentials.Add(new LJointmotionPairwisePixelPotential(this, crf, objDomain, baseLayer, motionDomain, motionBaseLayer, classNo, motionClassNo, pairwisePrior, pairwiseFactor, pairwiseBeta, pairwiseLWeight, pairwiseUWeight, pairwiseVWeight, disparityPairwiseFactor, disparityPairwiseTruncation, crossPairwiseWeight));
//	crf->potentials.Add(new LJointPairwisePixelPotential(this, crf, objDomain, baseLayer, motionDomain, motionBaseLayer, motionClassNo, disparityClassNo, pairwisePrior, pairwiseFactor, pairwiseBeta, pairwiseLWeight, pairwiseUWeight, pairwiseVWeight, disparityPairwiseFactor, disparityPairwiseTruncation, crossPairwiseWeight));

//    crf->potentials.Add(new LPairwisePixelPotential(this, crf, motionDomain, motionBaseLayer, motionClassNo));



    
//LDisparityUnaryPixelPotential *pixelDispPotential = new LDisparityUnaryPixelPotential(this, crf, motionDomain, motionBaseLayer, disparityClassNo, disparityUnaryFactor, disparityFilterSigma, disparityMaxDistance, disparityDistanceBeta, disparitySubSample, disparityMaxDelta, disparityLeftFolder, disparityRightFolder);
//crf->potentials.Add(pixelDispPotential);

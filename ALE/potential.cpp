...
void LDenseUnaryPixelPotential::Train(LList<char *> &trainImageFiles)
{
	...
	std::cout<<"totalFeatures "<<totalFeatures<<std::endl;
	...
	printf("multitread_problem\n");

	int processors = 1;
#ifdef MULTITHREAD
	processors = GetProcessors();
#endif
	printf("multitread_not_problem\n");

	featureValues = new int *[processors];
	//int *featureValues = new int(processors);
		std::cout<<"featureValues(1) "<<featureValues<<std::endl;

	for(i = 0; i < processors; i++)  {
		std::cout<<"numExamples(1) "<<numExamples<<std::endl;
		//numExamples=100;
		featureValues[i] = new int [numExamples];
		std::cout<<"featureValues(1) "<<featureValues[i]<<std::endl;
		}
...
//		if (j==0 && k==0) {		std::cout<<std::endl;
//for(l = 0; l < classNo; l++) std::cout<<data[l]<<" ";}
